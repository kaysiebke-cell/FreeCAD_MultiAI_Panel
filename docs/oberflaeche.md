...
**Toolbar buttons** toggle the 10 panels on and off (🧩 Snippets & API and 🧰 Tools & Library are each one panel with two sub-tabs; 🎤 Sprache is offline voice control — see [Voice Control](sprachsteuerung.md)). By default buttons show only the emoji (32 px). In the `♿ Help+Access` dock → `♿ Access` tab, labels can be enabled, e.g. `⚙ Settings`.
